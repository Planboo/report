...
export function Gallery() {
  ...
  // Subscribe to realtime updates for all photo collections
  const handleRealtimeUpdate = () => {
    refreshPhotos();
  };

  useRealtimeSubscription("cooks", "update", handleRealtimeUpdate, undefined, authState.isAuthenticated);
  useRealtimeSubscription("mixtures", "update", handleRealtimeUpdate, undefined, authState.isAuthenticated);
  useRealtimeSubscription("fields", "update", handleRealtimeUpdate, undefined, authState.isAuthenticated);

  const selectedItems = useMemo(
    () => photos.filter((p) => selected[p.source + ":" + p.id]),
    [photos, selected],
  );
    ...
    <div className="p-6">
      ...
      >
        <input
          ...
          placeholder={
            selectedItems.length
              ? `Set comment for ${selectedItems.length} items`
              : "Select photos to comment"
          }
          style={{
        ...
        <button
          ...
          disabled={!selectedItems.length || !comment}
          style={{
            background: "#111827",
            color: "white",
            border: 0,
