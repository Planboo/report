...
function buildFilter(
  collection: keyof typeof COLLECTION_FIELD_MAP,
  ...
  if (filters.dateFrom || filters.dateTo) {
    f[map.dateCreated] = {};
    if (filters.dateFrom)
      (f[map.dateCreated] as Record<string, unknown>)._gte = filters.dateFrom;
    if (filters.dateTo)
      (f[map.dateCreated] as Record<string, unknown>)._lte = filters.dateTo;
  }
  return f;
}
...
function normalize(
  ...
  return rows.map((row) => ({
    ...
    dateCreated: row[map.dateCreated]
      ? String(row[map.dateCreated])
      : undefined,
    ...
    directus.request(
      readItems(
        "cooks" as never,
        {
          ...
          fields: [
            "id",
            COLLECTION_FIELD_MAP.cooks.file,
            COLLECTION_FIELD_MAP.cooks.projectId,
            COLLECTION_FIELD_MAP.cooks.company,
            COLLECTION_FIELD_MAP.cooks.dateCreated,
            COLLECTION_FIELD_MAP.cooks.type,
            COLLECTION_FIELD_MAP.cooks.name,
          ],
          limit: 50,
          sort: ["-date_created"],
        } as never,
    ...
    directus.request(
      readItems(
        "mixtures" as never,
        {
          ...
          fields: [
            "id",
            COLLECTION_FIELD_MAP.mixtures.file,
            COLLECTION_FIELD_MAP.mixtures.projectId,
            COLLECTION_FIELD_MAP.mixtures.company,
            COLLECTION_FIELD_MAP.mixtures.dateCreated,
            COLLECTION_FIELD_MAP.mixtures.type,
            COLLECTION_FIELD_MAP.mixtures.name,
          ],
          limit: 50,
          sort: ["-date_created"],
        } as never,
    ...
    directus.request(
      readItems(
        "fields" as never,
        {
          ...
          fields: [
            "id",
            COLLECTION_FIELD_MAP.fields.file,
            COLLECTION_FIELD_MAP.fields.projectId,
            COLLECTION_FIELD_MAP.fields.company,
            COLLECTION_FIELD_MAP.fields.dateCreated,
            COLLECTION_FIELD_MAP.fields.type,
            COLLECTION_FIELD_MAP.fields.name,
          ],
          limit: 50,
          sort: ["-date_created"],
        } as never,
...
export async function bulkUpdateComments(
  items: Array<Pick<NormalizedPhoto, "id" | "source" | "commentField">>,
  comment: string,
): Promise<void> {
  ...
  for (const [collection, group] of groups.entries()) {
    ...
    promises.push(
      directus.request(
        updateItems(collection as never, ids, { [field]: comment } as never),
      ),
    );
  }
